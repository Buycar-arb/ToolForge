...
from Stage_3.core.api_client import APICaller
from Stage_3.core.mcp_client import MCPCaller
from Stage_3.services.data_processor import DataProcessor
from Stage_3.services.tool_manager import ToolManager
from Stage_3.services.conversation_generator import ConversationGenerator
from Stage_3.config.settings import DEFAULT_MODEL, DEFAULT_MAX_TOKENS, DEFAULT_TEMPERATURE

# === Validation-related modules ===
from Stage_4.config.api_keys import API_KEYS
from Stage_4.config.settings import API_CONFIG
from Stage_4.core.llm_client import AsyncLLMGenerateLabel
from Stage_4.validators.validation_engine import ValidationEngine
from Stage_4.utils.helpers import parse_llm_result
from Stage_4.prompts.end_judge_prompts import system_prompt, user_prompt
...
async def main():
    ...
    # Configuration for case C: case_name -> (target_count, validated_output_file, score_output_file)
    # Modify output paths to match your environment before running.
    c_cases_config = {
        'case_C1': (
            100,
            "output/validated/case_C1.jsonl",   # ← set your output path
            "output/scores/score_case_C1.jsonl" # ← set your score output path
        ),
        # Add more cases as needed, e.g.:
        # 'case_C3': (100, "output/validated/case_C3.jsonl", "output/scores/score_case_C3.jsonl"),
    }
    
    base_config = {
        'system_prompt_file': os.path.join(BASE_DIR, "Stage_3", "prompts", "opt_sys_prompt.txt"),
        'user_prompt_file': os.path.join(BASE_DIR, "Stage_3", "prompts", "opt_user_prompt.txt"),
        'tool_bank_file': os.path.join(BASE_DIR, "Stage_3", "tool_bank", "tools"),
        'virtual_tool_number_min': 3,
        'virtual_tool_number_max': 8,
        'max_tokens': 8192,
        'temperature': 0.0,
        'input_file': "Stage_2/label_data/output.jsonl",  # ← output from Stage 2
    }
