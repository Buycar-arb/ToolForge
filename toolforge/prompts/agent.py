"""The system / user prompt that ends up **inside the training data**.

Unlike every other prompt in this package (which drives the data *factory*),
these two are what the fine-tuned model itself will see at inference time:
:data:`AGENT_SYSTEM_PROMPT` becomes the ``system`` message of each generated
sample, and :data:`AGENT_USER_PROMPT` wraps the user question.
"""
...
#: ``{}`` is replaced with the original multi-hop question.
AGENT_USER_PROMPT = """\
The original question from the user is: {}"""
