"""Stage 4 — the LLM-judge prompt.

Rule checks catch structural defects; they cannot tell whether the *reasoning*
holds together.  This prompt asks a strong model to read every ``assistant``
turn and score think/action consistency: did the ``<think>`` block choose a tool
that fits the query, did the ``<tool_call>`` match what it said it would do, and
where a turn went wrong, did the next turn actually reflect on the mistake?

The verdict comes back as ``<score>\n[1]\n</score>`` or ``<score>\n[0]\n</score>``.
"""
